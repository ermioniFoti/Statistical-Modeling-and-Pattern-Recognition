...
figure(2)
plot(X(Y==0, 20), X(Y==0, 21), 'bo',X(Y==1, 20), X(Y==1, 21), 'ro' );
axis square;
title('Data Samples in 2D')
xlabel('Feature 20');
ylabel('Feature 21');
...
% ADD YOUR CODE
total_veriance = sum(eigvals);
ExplainedVar = eigvals / total_veriance;
...
% ADD YOUR CODE
ExplainedVar = diag(eigvals) / sum(diag(eigvals)) * 100;
fprintf(' Explained Variance(1st PC = %f) (2nd PC = %f)\n', ExplainedVar(1), ExplainedVar(2));
...
for K = [10, 50, 200]
    fprintf('\nDimension reduction for face dataset with K = %d.\n\n', K);

    % Project the data to K dimensions
    Z = projectData(X_norm, eigvecs, K);

    % Recover the data from the reduced dimensions
    X_rec = recoverData(Z, eigvecs, K);
    ...
    subplot(1, 2, 2);
    displayData(X_rec(1:100,:));
    title(sprintf('Recovered faces (K = %d)', K));
    axis square;
